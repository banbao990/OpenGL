...
vec2 ParallaxMapping3(vec2 texCoords, vec3 viewDir) {
    ...
    float beforeDepth = texture(depthTexture, prevTexCoords).r - currentLayerDepth + layerDepth; // before < 0?
...
void main() {
    ...
    if(parallaxMappingOn) {
         nTexCoords = ParallaxMapping3(fs_in.TexCoords,  viewDir);
         // 丢弃边缘的部分(纹理偏移后溢出)
