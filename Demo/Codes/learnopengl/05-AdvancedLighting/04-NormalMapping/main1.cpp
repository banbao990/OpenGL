...
// 例如, 你可以看到在每个砖块的顶部, 颜色倾向于偏绿, 这是因为砖块的顶部的法线偏向于指向正y轴方向(0, 1, 0)

// 严重的问题
// 如果物体不是朝向正z轴的, 但是此时法线仍然是朝向正z轴的, 会导致光线完全错误

// 解决方案
// (1) 为每个表面制作一个单独的法线贴图
// 如果是一个立方体的话我们就需要6个法线贴图
// 但是如果模型上有无数的朝向不同方向的表面这就不可行了
// (实际上对于复杂模型可以把朝向各个方向的法线储存在同一张贴图上, 你可能看到过不只是蓝色的法线贴图
// 不过用那样的法线贴图有个问题是你必须记住模型的起始朝向
// 如果模型运动了还要记录模型的变换, 这是非常不方便的
// 此外就像作者所说的, 如果把一个diffuse纹理应用在同一个物体的不同表面上
// 就像立方体那样的, 就需要做6个法线贴图, 这也不可取)

// (2) 切线空间
// 在一个不同的坐标空间中进行光照
// 这个坐标空间里,  法线贴图向量总是指向这个坐标空间的正z方向
// 所有的光照向量都相对与这个正z方向进行变换
// 这样我们就能始终使用同样的法线贴图, 不管朝向问题
