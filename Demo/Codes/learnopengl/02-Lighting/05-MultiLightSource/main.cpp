...
int main(int argc, char *argv[]) {
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        // spot light
        // 给一个余弦值, 方便比较
        shaders.setFloat("light.inCutOff", glm::cos(glm::radians(7.5f)));
        shaders.setFloat("light.outCutOff", glm::cos(glm::radians(12.5f)));

        // point light
        for (int i = 0; i < 4; ++i) {
            std::string a("pointLights[");
            a += std::to_string(i) + "].";
            shaders.setVec3(a + "position", pointLightPositions[i]);
            shaders.setVec3(a + "ambient", 0.05f, 0.05f, 0.05f);
            shaders.setVec3(a + "diffuse", 0.8f, 0.8f, 0.8f);
            shaders.setVec3(a + "specular", 1.0f, 1.0f, 1.0f);
            shaders.setFloat(a + "constant", 1.0f);
            shaders.setFloat(a + "linear", 0.09f);
            shaders.setFloat(a + "quadratic", 0.032f);
        }
